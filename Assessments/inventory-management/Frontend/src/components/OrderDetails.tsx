...
const OrderDetails = ({id, isSeller}:{id:string, isSeller:boolean}) => {
  ...
  return (
    ...
    <div className="flex flex-col justify-between gap-3">
      ...
      <p>
        <span className="font-medium">Total Price: </span> ${selectedOrder?.totalPrice}
      </p>
      ...
      <p className="flex gap-2 items-center">
        ...
        content={selectedOrder?.orderStatus.toString()
          .replace(/_/g, " ")
          .toLowerCase()
      ...
      <p className="flex gap-2 items-center">
       ...
       {selectedOrder?.paymentStatus && <StatusComponent
          bgColor={`${
          ...
          content={selectedOrder?.paymentStatus.toString()
            .replace(/_/g, " ")
            .toLowerCase()
            .replace(/(?<=\b)\w/g, (match: string) => match.toUpperCase())}
        />} 
    ...
    {!isSeller && selectedOrder?.orderStatus== "PENDING" && <div className="flex justify-end w-full">
        ...
        {isSeller && userDetails?.role == VENDOR && selectedOrderStatus &&  selectedPaymentStatus&& <div>
            ...
            <select defaultValue={selectedPaymentStatus} onChange={(e)=>setSelectedPaymenStatus(e.target.value)} className="px-2 py-1 rounded-lg bg-secondary-white border-1 font-normal text-xs">
