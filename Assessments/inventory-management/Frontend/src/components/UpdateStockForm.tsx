...
import { fetchProductById, patchProduct } from "../pages/product-list-page/productSaga";
...
const UpdateStockForm = ({id, closeModalCb}:{id:string, closeModalCb:Function}) => {

   const dispatch = useDispatch()
   const {isLoading, selectedProduct ,isSuccess} = useSelector((state:rootState)=>state.product)
    ...
    {selectedProduct?.quantity && <Formik
       ...
       onSubmit={(values) => {
        if(values.quantity !== selectedProduct.quantity){
            dispatch(patchProduct({id, formData:values  , dispatchAction:()=>dispatch(fetchAllLowStocks({page:1, perPage:10, category:""}))}));
     ...
     >
      <Form className="flex  w-fit"> 
        ...
        <Field className="px-2 py-1 bg-tertiary-gray rounded-lg color-primary-white" placeholder="Enter updated quantity"  id="quantity" name="quantity" type="number" required={true} autoComplete="off"/>
