      ...
      <div className="h-fit">
        <div className="h-fit w-full border-x-[1px] border-table-border bg-sidebar-bg overflow-x-scroll rounded-t-[5px] overflow-hidden">
          <div className="flex bg-header-bg border-table-border border-y-[1px] text-text-dark justify-between items-center  p-2 w-full min-w-fit">
            ...
            {isActions && (
              <span className="text-center font-medium w-[100px]">Actions</span>
            )}
          </div>

          <div className="">
            {tableData.length == 0 && (
              <div className="flex justify-center items-center bg-primary-white p-2  border-table-border border-b-[1px] w-full min-w-fit duration-100 ease-in-out">No Data Available</div>
            )}
            {isLoading ? (
              <Loader />
            ) : (
                ...
                (
                  ...
                  return (
                    ...
                    >
                      {Object.values(row).map((data: any, key: number) => {
                        return (
                          <span
                            onClick={(e) => {
                              e.stopPropagation();
                              onClickRowCb && key == 1 && onClickRowCb(row.id);
                            }}
                            style={{
                            ...
                            className={clsx(
                              `text-left ${
                                key == 1 &&
                                onClickRowCb &&
                                "cursor-pointer  hover:underline-offset-4 hover:text-dark-orange duration-100 ease-in-out"
                              }`
                            )}
                          >
                            {key == 0 && columns[0].name == "Sr no."
                      ...
                      {isActions && (
                        <span className="text-center font-medium w-[100px] flex justify-center items-center gap-3">
                          {actions?.map((action, key) => (
                            <span
                              className="cursor-pointer"
                              key={key}
                              onClick={() => {
                                action.onClickCb(row.id);
                              }}
                            >
                              {action.content}
                            </span>
