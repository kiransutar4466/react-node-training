...
export const getAllVendorsService = async (
  ...
  try {
    ...
    const response = await axios.get(
      `${import.meta.env.VITE_SERVER_BASE_URL}vendors?page=${payload.page}&perPage=${payload.perPage}&search=${payload.searchQuery}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export const getVendorByIdService = async (payload: string) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_SERVER_BASE_URL}vendors/${payload}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "ngrok-skip-browser-warning": "69420",
...
export const patchVendorService = async (payload: {
  id: string;
  ...
  try {
    const response = await axios.patch(
      `${import.meta.env.VITE_SERVER_BASE_URL}vendors/${payload.id}`,
      payload.formData,
      {
        headers: {
          Authorization: `Bearer ${token}`,
