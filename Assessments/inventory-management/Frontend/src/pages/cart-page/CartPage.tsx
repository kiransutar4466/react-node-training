    ...
    <>
      ...
      <div className="w-full h-full max-h-full overflow-y-scroll px-5">
        ...
        {cart && (
          <>
            {cart?.length > 0 && (
              <div className="flex w-full justify-between gap-3 my-2 mt-12">
                <Button
                  onClickCb={() => {
                    setIsModalVisible(true);
                    setModalReason("emptyCart");
                  }}
                  btnContent={"Empty Cart"}
                  bgColor="button-blue"
                  color="primary-white"
                  width="fit"
                />
            ...
            <div className="w-full border-x-[1px]  border-table-border bg-sidebar-bg overflow-x-scroll rounded-t-[5px] min-h-fit">
              <div className="w-full flex justify-between gap-2 px-5 text-start  h-10 items-center font-medium bg-header-bg border-table-border border-y-[1px] text-text-dark">
                ...
                <span className="w-[200px]"> Product Name</span>
                ...
                <span className="w-[100px] text-center flex justify-center items-center">
                  Actions
                </span>
              </div>
              {cart && cart.length > 0 ? (
                cart?.map((cartItem, key) => (
                  <div
                    key={key}
                    className="w-full flex justify-between gap-2 p-5 text-start bg-primary-white  my-2   border-table-border border-b-[1px]  min-w-fit "
                  >
                    <span className="w-[50px]">{key + 1}</span>
                    <span className="w-[200px]"> {cartItem.productName}</span>
                    <span className="w-[300px]">
                      {" "}
                      {cartItem.productDescription.slice(0, 30)}{" "}
                      {cartItem.productDescription.length > 30 && "..."}
                    </span>
                    ...
                    <div className="w-[100px] flex justify-around bg-secondary-white rounded-xl items-center">
                      <span
                        className={`cursor-pointer ${cartItem.quantity > 1 ? "text-primary-black" : "text-secondary-gray"}`}
                        onClick={() => {
                          cartItem.quantity > 1 &&
                            dispatch(
                              patchCart({
                    ...
                    <span
                      className="w-[100px] text-center flex justify-center items-center cursor-pointer text-[20px] relative"
                      title="Remove Item"
                      onClick={() => {
                        dispatch(
                          deleteCartItem({
                ...
                <span className="flex justify-center items-center border-table-border border-b-[1px] py-2 bg-primary-white">
                  Your cart is empty.
                </span>
              )}
            </div>
