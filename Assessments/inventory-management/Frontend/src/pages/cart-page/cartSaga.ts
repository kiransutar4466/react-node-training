...
export const fetchCart = (payload: { page: number; perPage: number }) => {
...
function* getCart(action: {
  ...
  const response: { data?: any; error?: string } = yield getCartService(
    action.payload,
  );
...
function* editCart(action: {
  ...
  const response: { data?: any; status?: number; error?: string } =
    yield patchCartService(action.payload);
...
function* postCart(action: {
  type: string;
  payload: { productId: string; quantity: number; dispatchAction: Function };
}) {
  yield put(startCartApiRequest());

  const response: { data?: any; status?: number; error?: string } =
    yield postCartItemService(action.payload);
...
function* deleteCartItemSaga(action: {
  ...
  const response: { data?: any; status?: number; error?: string } =
    yield deleteCartItemService(action.payload.id);
