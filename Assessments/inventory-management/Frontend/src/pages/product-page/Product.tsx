...
const Product = () => {
  ...
  const cartItem = cart?.filter((item) => item.productId == id)[0];
  ...
  useEffect(() => {
    selectedProduct?.name &&
      getUnsplashImage(selectedProduct?.name).then((url) => setImageUrl(url));
  }, [selectedProduct]);
      ...
      <div className=" w-full h-full p-5 px-10">
        <div className="my-3 flex flex-col justify-start gap-1 items-start relative">
          <span
            ...
            className="absolute top-1 left-[-30px] cursor-pointer "
          >
            <FaArrowLeft />
          </span>
