...
import {
  failProductApiRequest,
  startProductApiRequest,
  successAddProduct,
  successEditProduct,
  successFetchAllCategories,
  successFetchAllProducts,
  successFetchProductById,
  successDeleteProductById,
} from "./productSlice";
import {
...
export const patchProduct = (payload: {
  id: string;
...
function* getAllProducts(action: {
  ...
  const response: { data?: any; status?: number; error?: string } =
    yield getAllProductsService(action.payload);
...
function* getAllCategories(action: { type: string; payload: number }) {
  ...
  const response: { data?: any; status: number; error?: string } =
    yield getAllCategoryService(action.payload);
...
function* getProductById(action: { type: string; payload: string }) {
  ...
  const response: { data?: any; status?: number; error?: string } =
    yield getProductByIdService(action.payload);
...
function* editProduct(action: {
  ...
  const response: { data?: any; error?: string } = yield patchProductService(
    action.payload,
  );
...
function* postProduct(action: {
  type: string;
  payload: { formData: ProductFormInputType; dispatchAction?: Function };
}) {
  yield put(startProductApiRequest());

  const response: { data?: any; error?: string } = yield postProductService(
...
function* deleteProductById(action: {
  ...
  const response: { data?: any; error?: string } =
    yield deleteProductByIdService(action.payload.id);
