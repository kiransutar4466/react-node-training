...
export const fetchAllLowStocks = (payload: {
  page: number;
  perPage: number;
  category: string;
}) => {
...
function* getAllLowStocks(action: {
  type: string;
  payload: { page: number; perPage: number; category: string };
}) {
  yield put(startLowStocksApiRequest());

  const response: { data?: any; status: number; error?: string } =
    yield getAllLowStocksService(action.payload);
