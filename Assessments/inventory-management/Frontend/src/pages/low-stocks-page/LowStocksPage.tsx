...
const LowStocksPage = () => {
  ...
  const { lowStocks, isLoading, details } = useSelector(
    (state: rootState) => state.lowStocks,
  );
      ...
      <div className="w-full h-full max-h-full overflow-y-scroll px-5">
        <div className="my-3">
          ...
          <p className="text-sm cursor-pointer">
            ...
            <span onClick={() => navigate("/low-stocks")}>Dead Stocks</span>
          </p>
        </div>
        ...
        <Modal
          ...
          childComponent={
            <Suspense fallback={<SmallLoader />}>
              <UpdateStockForm
                id={selectedId}
                closeModalCb={() => setIsModalVisible(false)}
              />
            </Suspense>
          }
        />
        ...
        {lowStocks && data && (
          <Table
            ...
            tableData={data}
            columns={columns}
            isActions={userDetails?.role == VENDOR ? true : false}
            actions={[
              {
                content: <MdEdit />,
                onClickCb: (id: string) => {
